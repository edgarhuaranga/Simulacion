...
//      *** Variable definitions and initializations ***
        int M=4;   int N=10;   double t=0;  double T=100;
        int [] SS = new int [M];   double [] EL = new double [M];
        double [] mu = {3, 1, .75, 2};
...
//     *** Begin  simulation proper ***
        ...
        while (t <=  T) {
            ...
            int source = 0;    double small = EL[0];            // Get source
            for (int i=1; i<M; i++) {
                if (small > EL[i]) { small = EL[i]; source = i;}
            }
            u = generator.nextDouble();                         //  Get destination
            int dest = 0;   while (u > RR[source][dest]) dest++;

            t = EL[source];  SS[source]--;  SS[dest]++;         // Next state
            EL[source] = infty;
            if (SS[source] > 0) {
